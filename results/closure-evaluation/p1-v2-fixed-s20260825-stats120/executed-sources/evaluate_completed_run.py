...
def main():
    ...
    online_required = is_p0 or args.run == "p1-v2-fixed-s20260825-stats120"
    ...
    if args.run == "p1-v2-fixed-s20260825-stats120":
        from run_p1_v2_fixed import validate_train_args

        frozen = json.loads(
            (ROOT / "configs/p1-v2-fixed-s20260825.train.json").read_text(encoding="utf-8")
        )["train"]
        validate_train_args(frozen, payload["train_args"])
        validate_train_args(frozen, healthy["train_args"])
    del healthy, payload
    work = WORK / args.run
    ...
    if online_required:
        ...
        subprocess.run(
            [
                ...
                str(out / "figures"),
                "--label",
                "P0 locked baseline" if is_p0 else "P1 v2 fixed, seed 20260825",
            ],
            check=True,
            timeout=300,
        )
