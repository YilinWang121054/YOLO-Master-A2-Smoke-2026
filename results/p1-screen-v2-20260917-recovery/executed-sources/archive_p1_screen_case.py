...
def validate_child_logs(logs):
    """An interrupted attempt is provenance, never a successful evaluation."""
    launches = sorted(logs.glob("*.launch.json"))
    if not launches:
        raise ValueError("No original child launch logs")
    interrupted = []
    for launch in launches:
        related = {
            suffix: launch.with_name(launch.name.replace(".launch.json", suffix))
            for suffix in (".stdout.log", ".stderr.log", ".exit.json", ".interrupted.json")
        }
        if any(not related[s].is_file() for s in (".stdout.log", ".stderr.log")):
            raise ValueError("Missing original child log")
        if related[".exit.json"].exists():
            if related[".interrupted.json"].exists():
                raise ValueError("Ambiguous exit and interruption records")
            continue
        receipt_path = related[".interrupted.json"]
        if not receipt_path.exists():
            raise ValueError("A child log has no recorded exit; defer archive for inspection")
        ...
        if (
            ...
            or any(receipt.get(k) != event[k] for k in ("worker", "case", "pid", "started_at"))
        ):
            raise ValueError("Invalid interruption receipt; no exit code may be invented")
        ...
        if (
            ...
            raise ValueError("Missing shutdown/boot event evidence")
        times = [datetime.fromisoformat(t) for t in (
            event["started_at"], shutdown["time"], reboot["time"], receipt["observed_at"]
        )]
        if any(t.utcoffset() is None for t in times) or not all(a < b for a, b in zip(times, times[1:])):
            raise ValueError("Interruption chronology is invalid")
        interrupted.append(receipt_path.name)
    return interrupted
...
def main(argv=None):
    ...
    interruptions = validate_child_logs(logs)
    ...
    save(
        out / "archive-verification.json",
        {
            ...
            "interrupted_attempt_receipts": interruptions,
            "interruption_scope": "Historical incomplete attempts only; five successful evaluations remain mandatory and independently hash-verified",
            "excluded": [
                "dataset images",
                "GT annotations",
                "checkpoint binaries",
