...
def test_uninterrupted_run_preserves_bytes_and_records_absent_recovery(run_files):
    root, project, name, _initial = run_files
    archive.main(["--run", name])
    manifest = json.loads(
        (
            root / "results/closure-evaluation" / name / "training-log-manifest.json"
        ).read_text()
    )
    ...
    assert not (
        root / "logs/closure-evaluation" / name / "training-original/recovery"
    ).exists()
    for record in manifest["records"]:
        assert (root / record["archived"]).read_bytes() == Path(
            record["source"]
        ).read_bytes()
...
def test_resumed_run_keeps_failure_logs(run_files):
    ...
    copied = (
        root
        / "logs/closure-evaluation"
        / name
        / "training-original/recovery/attempt.stderr.log"
    )
    assert copied.read_bytes() == b"original failure\r\n"


@pytest.mark.parametrize("json_only", [False, True])
def test_existing_recovery_directory_without_logs_is_not_silently_skipped(
    run_files, json_only
):
    root, project, name, _initial = run_files
    recovery = project / "recovery-logs" / name
...
def test_different_existing_archive_is_not_overwritten(run_files):
    root, _project, name, _initial = run_files
    copied = (
        root
        / "logs/closure-evaluation"
        / name
        / "training-original/initial/initial.stdout.log"
    )
...
def test_v2_uses_its_single_launch_and_resume_directory(tmp_path):
    recovery = tmp_path / "recovery-logs" / archive.V2_NAME
    recovery.mkdir(parents=True)
    (recovery / "initial.stdout.log").write_bytes(b"initial\r\n")
    (recovery / "resume.stdout.log").write_bytes(b"resumed\r\n")
    sources = archive.original_sources(tmp_path, archive.V2_NAME)
    assert len(sources) == 1
    assert sources[0][0] == "recovery"
    assert len(sources[0][2]) == 2


def test_v2_missing_original_logs_is_rejected(tmp_path):
    with pytest.raises(ValueError, match="Required original log directory missing"):
        archive.original_sources(tmp_path, archive.V2_NAME)
