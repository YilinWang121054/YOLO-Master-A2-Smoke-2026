...
def running_processes(experiment_id: str, run_dir: Path) -> list[int]:
    ...
    for process in psutil.process_iter(["pid", "name", "cmdline"]):
        ...
        joined = " ".join(lowered)
        is_yolo_train = "train" in lowered and any(
            Path(item.strip('"')).name.lower() == "yolo.exe" for item in cmdline
        )
...
def inspect_checkpoint(path: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, expected in config.get("expected_train_args", {}).items():
        if train_args.get(name) != expected:
            raise ValueError(
                f"checkpoint protocol drift: {name}={train_args.get(name)!r}, expected {expected!r}"
            )
    minimum_epoch = -1 if config.get("allow_bootstrap_checkpoint", False) else 0
    if not isinstance(epoch, int) or not minimum_epoch <= epoch < int(
        config["expected_epochs"]
    ):
    ...
    if not data_path.exists():
        raise FileNotFoundError(
            f"dataset config recorded by checkpoint is missing: {data_path}"
        )
    return {
        "path": str(path),
...
def select_checkpoint(
    ...
    for candidate in candidates:
        ...
        try:
            ...
            errors.append(
                {"path": str(candidate), "error": f"{type(error).__name__}: {error}"}
            )
    return None, errors


def rewind_csv(
    path: Path, header: list[str], rows: list[dict[str, str]], keep_rows: int
) -> Path:
...
def main() -> int:
    ...
    if len(rows) >= expected_epochs:
        if config.get("require_online_assignment"):
            missing = missing_assignment_epochs(run_dir, expected_epochs)
            if missing:
                return emit(
                    "refused",
                    reason="online assignment evidence is incomplete",
    ...
    if checkpoint is None:
        return emit(
            "refused",
            reason="no valid resumable checkpoint",
    ...
    if config.get("require_online_assignment"):
        missing = missing_assignment_epochs(run_dir, checkpoint_epochs)
        if missing:
            return emit(
                "refused",
                reason="checkpoint is ahead of online assignment evidence",
                missing_epochs=missing,
            )
    csv_backup = None
    ...
    if config.get("resume_runner"):
        runner = Path(config["resume_runner"]).resolve()
        if not runner.is_file():
            return emit(
                "refused", reason=f"instrumented resume runner is missing: {runner}"
            )
        command = [
    ...
    environment["PYTHONPATH"] = (
        str(workspace) + os.pathsep + environment.get("PYTHONPATH", "")
    )
    creationflags = 0
    if os.name == "nt":
        creationflags = (
