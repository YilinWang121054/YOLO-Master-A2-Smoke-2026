...
def main():
    ...
    if not args.allow_partial and len(completed) != 120:
        raise ValueError("Final P0 figure requires all 120 completed epochs")
    ...
    for branch in branches:
        ...
        fig.suptitle(
            f"P0 locked baseline: assignment evolution ({len(completed)}/120 epochs)",
            fontsize=10,
            y=0.97,
        )
        fig.text(
    ...
    caption = (
        f"P0 正样本演化，已完成 {len(completed)}/120 epoch。曲线为各轮训练增强后 small/medium/large GT "
