...
$a2Python = 'F:\conda-envs\yolo-master\pythonw.exe'
...
foreach ($a2Worker in @('gpu', 'cpu')) {
    $a2Task = 'YOLO-Master-A2-Screen-V2-' + $a2Worker.ToUpper()
    $a2Args = '-X utf8 "{0}" --worker {1}' -f $a2Script, $a2Worker
    $a2Action = New-ScheduledTaskAction -Execute $a2Python -Argument $a2Args -WorkingDirectory $PSScriptRoot
