...
V2_NAME = "p1-v2-fixed-s20260825-stats120"
...
def original_sources(project, name):
    ...
    if name == V2_NAME:
        directories = (("recovery", project / "recovery-logs" / name),)
    for label, source in directories:
        if label == "recovery" and name not in (NAME, V2_NAME) and not source.exists():
...
def main(argv=None):
    ...
    if name in (NAME, V2_NAME) and missing:
    ...
    for label, source, files in original_sources(PROJECT, name):
        source_directories.append(
            {
                ...
                "note": (
                    ...
                    if files
                    else "No recovery directory observed; no recovery logs supplied. "
        ...
        for original in files:
            ...
            if copied.exists() and sha256(copied) != digest:
                raise ValueError(
                    f"Refuse to replace different archived bytes: {copied}"
                )
