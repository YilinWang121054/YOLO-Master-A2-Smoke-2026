...
def main():
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "p1-tal-s20260826",
            "p1-v2-fixed-s20260825-stats120",
        ),
    )
    ...
    evaluation = json.loads(
        (out / "evaluation-manifest.json").read_text(encoding="utf-8")
    )
    ...
    names = {
        p.name
        for p in Path("F:/datasets/VisDrone/VisDrone2019-DET-val/annotations").glob(
            "*.txt"
        )
    }
    ...
    if not zipped.exists():
        with zipfile.ZipFile(
            zipped, "x", compression=zipfile.ZIP_DEFLATED, compresslevel=6
        ) as archive:
            for path in files:
    ...
    if not compressed.exists():
        with (
            predictions.open("rb") as source,
            compressed.open("xb") as destination,
            gzip.GzipFile(
    ...
    manifest = {
        "run": name,
        "scope": "Predictions only; original images and GT are not included",
        "images": 548,
        "det_files_sha256": hashes,
        "det_zip_sha256": sha(zipped),
        "det_zip_bytes": zipped.stat().st_size,
        "predictions_gzip_sha256": sha(compressed),
        "predictions_gzip_bytes": compressed.stat().st_size,
        "predictions_uncompressed_sha256": evaluation["predictions_sha256"],
        "runtime_parity": "original MATLAB execution still pending",
    }
    (out / "prediction-archive-manifest.json").write_text(
