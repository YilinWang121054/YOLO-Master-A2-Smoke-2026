...
def values(det, area):
    return [
        f"{(det if index < 4 else area)[key]:.4f}" for index, key in enumerate(KEYS)
    ]
...
def check_completed_v2(text):
    folder = ROOT / "results/closure-evaluation/p1-v2-fixed-s20260825-stats120"
    if not folder.exists():
        return None
    note = ROOT / "docs/closure-review/P1第二轮fixed完成-20260915.md"
    note_text = note.read_text(encoding="utf-8")
    row = (
        "| "
        + " | ".join(
            values(
                read(folder / "det-source-metrics.json")["metrics"],
                read(folder / "coco-area-metrics.json")["metrics"],
            )
        )
        + " |"
    )
    if text.count(row) != 1 or note_text.count(row) != 1:
        raise ValueError("Completed v2 fixed metric row differs or is duplicated")
    assignment = read(folder / "assignment/epoch-120.json")["branches"][
        "topk10_topk210"
    ]
    for size in ("small", "medium", "large"):
        item = assignment[size]["after_conflict"]
        row = f"| {size} | {item['n']} | {item['mean']:.4f} | {item['p50']} | {item['p90']} | {100 * item['zero_fraction']:.4f}% |"
        if note_text.count(row) != 1:
            raise ValueError(f"Completed v2 assignment row differs for {size}")
    check_local_links(note)
    return {
        "note_sha256": hashlib.sha256(note.read_bytes()).hexdigest(),
        "report_numeric_cells": 8,
        "note_numeric_cells": 23,
    }
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "results/report-table-verification-20260911.json",
    )
    ...
    for line in text.splitlines():
        ...
        if len(cells) == 10 and re.fullmatch(r"2026082[456]", cells[0]):
            ...
            if cells[0] == "20260826":
                ...
                det = read(ROOT / f"results/official-det-strict-audit-{run}.json")[
                    "metrics"
                ]
    ...
    p0_row = (
        ...
        + " | ".join(
            values(
                read(p0 / "det-source-metrics.json")["metrics"],
                read(p0 / "coco-area-metrics.json")["metrics"],
            )
        )
        + " |"
    )
    ...
    v2 = check_completed_v2(text)
    result = {
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "report_sha256": hashlib.sha256(REPORT.read_bytes()).hexdigest(),
        "comparison_rows": len(checked),
        "checked_numeric_cells": len(checked) * 8 + 8 + 15 + (31 if v2 else 0),
        "completed_v2": v2,
