...
版本：2026-09-09 本地审阅稿。长训和 P0 补充证据尚未全部完成，本文不是已通过验收的结题定稿。
...
历史成果补充细则指定的公共基线为 `acce839c7e895d6b179de7f7093fa879e237cc7b`。本轮训练使用的 A2 提交是 `52c2befa50706b9dff13b6e0813b19413d9f532d`，其直接上游是 `0996b7da14dfaafae9d4488e960814ff19eb19ce`。两个上游版本间的 30 个提交属于社区已有工作。个人功能改动位于 TAL、检测 loss 的配置注入、默认配置与校验、STAL 测试这 5 个文件中。
...
9 月 9 日核查发现，COCO evaluator 的默认区间包含两端，恰好位于阈值的目标会同时落入两档。原 val 有 13 个面积为 1024 的有效 GT、1 个面积为 9216 的 GT。修正边界后，使用原预测重评了全部六组；下表采用重评结果。此前公开文件保留用于追溯，不混入本表。DET 严格审计还找到两条实际预测的 ignore 过滤决定会受 MATLAB 取整影响，目前不改写早期总体表，等严格审计和老师口径确定后统一更新。
...
以下指标单位均为百分数；差值用绝对百分点。总体列为前述 DET Python 移植结果，三档列为修正边界后的 COCO-style 补充结果。

| Seed | 模式 | DET AP | AP50 | AP75 | AR500 | APs | APm | APl | ARs@500 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 20260824 | fixed | 22.4055 | 40.5716 | 21.4056 | 39.0019 | 13.5280 | 31.9935 | 39.4703 | 29.1766 |
| 20260824 | adaptive | 22.4328 | 40.3226 | 21.4831 | 38.5890 | 13.3655 | 32.0546 | 40.5131 | 28.8453 |
| 20260824 | pure TAL* | 21.5406 | 38.7786 | 20.3948 | 37.8632 | 12.6040 | 31.2151 | 39.5915 | 27.7936 |
| 20260825 | fixed | 21.7619 | 39.4406 | 20.7115 | 38.1235 | 13.0766 | 31.0920 | 39.4663 | 28.4817 |
| 20260825 | adaptive | 21.4915 | 38.9398 | 20.1943 | 37.7670 | 12.7361 | 30.4258 | 36.7081 | 27.9816 |
| 20260825 | pure TAL | 21.5139 | 38.6953 | 20.3479 | 37.9989 | 12.7167 | 31.0028 | 42.0973 | 28.3010 |
...
seed 20260826 的 fixed 已完成 120e，adaptive 仍在训练，pure TAL 待运行。fixed 的训练器 mAP50-95=0.20571 仅是内置验证指标，不填入 DET AP 或 APs 列。第三对完成后再补主表，当前不填估计值。
...
长训的负结果至少留下三个需要检验的解释。其一，新增候选可能较多但 target score 很低，实际监督增量有限。其二，密集区域中候选重叠增加，会让一部分 GT 在冲突后重新失去候选。其三，Mosaic 改变训练目标面积，与原图评测分档不一一对应。当前没有完整的长训逐 epoch 分配记录，因此这些是待检验解释，不是已证明的原因。

新采集器已在原始锁定代码上完成 CPU 训练烟测，并通过输出逐位一致与 RNG 状态不变的测试。它记录真实增强后的 GT，分别统计原框候选、扩展后候选、冲突前和冲突后数量，保存每档均值、P50/P90、零正样本比例。每轮统计与完成的 train batch 数校验；验证调用不计入。接下来的完整 P0 补跑会提供逐 epoch 演化数据。事后固定 checkpoint 探针即使补做，也将另行标记，不能补造原长训历史。

## 工程核对与剩余工作

训练 checkpoint 保存在 F 盘。9 月 9 日 adaptive 恢复后，控制器因重复传入 status 报错，GPU 训练本身仍继续，串行监督进程则退出。已修复参数冲突，增加监督锁，并禁止在半建成目录上再次 fresh launch，避免自动生成带后缀的新实验目录。9 月 10 日已按“P0 优先”调整队列，暂停 adaptive 并保留其 checkpoint，先启动原始基线 P0。P0 首轮已完成，在线统计文件 `assignment/epoch-001.json` 已生成；完成后再恢复 adaptive。

独立审阅副本中的 STAL、默认配置、模型配置测试为 31 passed，证据采集、恢复和面积边界测试为 11 passed，MATLAB-source DET fixture 为 5 passed。此次还补上 NaN/Inf 参数拒绝测试；正在训练的源代码保持在 52c2bef。文件级 lint 报出的 15 项经与直接上游对照，均已在上游存在，没有新增诊断；这不等于整个仓库 lint 已全部通过。新增测试文件的格式检查也已整理。

P0 当前正在补原始锁定基线的完整 120 轮正样本记录及其最终评测。P1 尚缺同协议三 seed 主表、训练期统计与 Mosaic 交互，已完成的两对数值提升目标也没有达到。P2 的阈值、warmup 扫描及第二数据集/任务尚未完成。时间安排首先保证 P0，评优需以导师确认的实际贡献口径为准。

## 复现与资料

公开证据仓库：[YOLO-Master-A2-Smoke-2026](https://github.com/YilinWang121054/YOLO-Master-A2-Smoke-2026)。已有社区进度：[Issue #246](https://github.com/Tencent/YOLO-Master/issues/246)。本次审阅文件尚未上传，以下路径指本地待审核材料。

- 基线和协议审计：`results/closure-audit-20260909.json`，脚本 `scripts/collect_closure_audit.py`。
- 修正后的六组分档指标：`results/area-audit-20260909/`，脚本 `scripts/recompute_area_audit.py`。
...
- 贡献 diff：公共基线→最终提交用于总审计；个人变更必须剔除中间 30 个上游提交。FINAL_REF 在结果、代码和用户审核完成后确定。

本课题使用公开 VisDrone 数据，数据与许可仍按原发布方要求获取。研究代码和报告由 AI 工具协助实现、核查与整理，最终内容由作者审阅后提交；工具生成文字不作为实验结果来源。实际数字均可回溯到预测、日志或 checkpoint。没有将尚未运行的实验记为完成。
