...
版本：2026-09-11，补充评测源码对照及实验分组说明，后续按作者审核意见更新。原始基线的120轮训练、逐轮正样本记录和完整验证已经完成；DET已补交源码级核验证据，尚无原MATLAB执行记录。现有结果不支持P1提升目标，P2尚未完成。
...
9月11日按允许的源码核验替代方案，补交了[逐项对照与边界测试](DET源码逐项对照-20260911.md)。P0同一预测的Python重算与原记录完全一致；这支持当前实现的可复现性，不等同于MATLAB/Python实测差值为零。精确版本、完整命令和输入哈希见该说明及其审计输出。
...
导师规定的 P1 条件同时要求统一协议下平均 ΔAPs≥+1.0、至少 2/3 seed 正向。现有两对均为负，且第一对已排除出正式重复实验，因此不能把当前表格用于P1达标判定。第三对仍按冻结配置完成，后续统一协议补跑若未完成就列为缺项，不通过挑seed或事后调参改变验收规则。

9月11日13:52，seed 20260826的adaptive完成120轮及CPU完整评测；APs相对同seed fixed增加0.3930个百分点，总体AP减少0.0334、APl减少0.6630。它是一个配对描述性结果，不是统一协议三seed的P1验收均值。pure TAL同seed已自动启动，尚无最终指标。第三seed的fixed/adaptive与下面的P0基线均使用CPU完成FP32推理；前两seed的保存预测来自GPU。各组保留推理设备和预测哈希，不将CPU/GPU数值等价当作未经检查的前提。训练器内置mAP不代替上述DET或面积指标。
...
P0的训练、逐轮统计、分档结果和原始日志已经收齐，DET源码级对照已补交待review。P1尚缺同协议三seed主表、对应训练期统计与Mosaic交互，现有参考结果也未显示目标提升。P2的阈值、warmup扫描及第二数据集/任务尚未完成。已合并的#253单列为工程贡献，不以代码合并或smoke通过替代量化要求。后续依次完成第三seed、Mosaic-off交互和小规模候选质量诊断，具体见[复现入口与后续安排](P0复现入口与后续安排-20260911.md)。
