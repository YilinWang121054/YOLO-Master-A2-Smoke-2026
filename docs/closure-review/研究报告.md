...
版本：2026-09-13，补齐seed 20260826三模式的120轮结果，后续按作者审核意见更新。原始基线的120轮训练、逐轮正样本记录和完整验证已经完成；DET已补交源码级核验证据，尚无原MATLAB执行记录。现有结果不支持P1提升目标，P2尚未完成。
...
P0的训练、逐轮统计、分档结果和原始日志已经收齐，DET源码级对照已补交待review。P1尚缺同协议三seed主表、对应训练期统计与Mosaic交互，现有参考结果也未显示目标提升。P2的阈值、warmup扫描及第二数据集/任务尚未完成。已合并的#253单列为工程贡献，不以代码合并或smoke通过替代量化要求。现有第三seed队列已结束，下一步是Mosaic-off交互和小规模候选质量诊断；前者的最低训练预算仍待确认，具体见[复现入口与后续安排](P0复现入口与后续安排-20260911.md)。
