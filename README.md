...
9月17日15:55，[第五组adaptive-minextent完成全部评测](docs/closure-review/P1第二轮筛选-minextent结果-20260917.md)：最后5轮平均APs为6.7754，相对同预算fixed提高0.3337个百分点，但APl下降2.5261个百分点，large零正样本从3次变为5次，仍不能入选。至此五组训练和25/30次评测完成，最后一组在本次开机后自动从第11轮checkpoint恢复，继续训练；本轮筛选尚未结束。

9月17日，[关机后的断点恢复检查](docs/closure-review/P1第二轮筛选-关机后恢复-20260917.md)已完成。前五组20轮训练产物复核通过，最后一组从第9轮checkpoint恢复第10轮；CPU已完成23/30次评测，其余按原定内存门槛排队。关机中断和启动失败的原始记录均保留，未修改冻结协议。完整筛选及P1仍未完成。
...
23:17，[adaptive-compact（scale=1.25、k10）完成五次评测](docs/closure-review/P1第二轮筛选-compact结果-20260915.md)，原始证据随后归档。最后5轮平均APs为6.9234，相对fixed提高0.4817个百分点，但APl下降1.7148个百分点，medium/large零正样本次数分别从46/3变为47/5，仍不满足筛选条件。目前四组已完成训练和评测，剩余两组按冻结队列继续。
